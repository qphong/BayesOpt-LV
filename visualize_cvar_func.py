...
func_name = "negative_rescaled_hartmann6d_15"
...
_, _, _, _, ground_truth_cvar_at_est_max = ground_truth_cvaropt.find_cvar(x_plc)
...
print(
    "VaR range: {} - {}".format(
        np.min(ground_truth_cvar_at_est_max_np), np.max(ground_truth_cvar_at_est_max_np)
    )
)


if xdim == 1:
    plt.plot(x_np, ground_truth_cvar_at_est_max_np)
elif xdim == 2:
    side = int(np.sqrt(ground_truth_cvar_at_est_max_np.shape[0]))
    # plt.imshow(ground_truth_cvar_at_est_max_np.reshape(side,side))
    X = np.tile(np.linspace(0.0, 1.0, side).reshape(side, 1), reps=(1, side))
    plt.contourf(X, X.T, ground_truth_cvar_at_est_max_np.reshape(side, side))
    plt.colorbar()
elif xdim == 3:
    ...
    for seed in range(5):
        ...
        cb = ax.contourf(
            ...
            ground_truth_cvar_at_est_max_np[:900].reshape(30, 30),
        )
        plt.colorbar(cb)
    ...
    plt.contourf(
        ...
        ground_truth_cvar_at_est_max_np.reshape(100, 100),
    )
    plt.colorbar()
